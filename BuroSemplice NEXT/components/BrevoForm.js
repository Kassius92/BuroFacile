...
export default function BrevoForm({ pageName = 'default' }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setMsg({ type: 'ok', text: 'Iscrizione inviata! Controlla la mail per confermare.' });
      setSent(true);
  ...
  return (
    <div className="remind-box">
      <div className="remind-inner">
        ...
        <div className="remind-title">Non perderti le scadenze</div>
        <div className="remind-desc">Ti mandiamo un promemoria prima di ogni scadenza importante. Gratis, niente spam.</div>
        ...
        {!sent && (
          <>
            <div className="remind-row">
              ...
              />
              <button className="remind-btn" onClick={handleSubmit}>Ricordami</button>
            </div>
        ...
        <div className="remind-note">Niente spam, solo scadenze. <a href="/privacy">Privacy policy</a></div>
      </div>
    </div>
  );
}
